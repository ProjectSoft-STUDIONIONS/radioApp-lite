module.exports = function(grunt) {
	...
	grunt.registerMultiTask('parse_record', 'Parse Radio Record stations', async function() {
		var done = this.async();
		options = this.options();
		if(options.update == true) {

			let mdFile = path.join(`.`, `radio.md`),
				m3u8File = path.join(`.`, `radio.m3u8`),
				readmeFile = path.join(`.`, `README.md`),
				dataJsonFile = path.join(`.`, `application`, `radio`, `data.json`),
